...
<header class="theme-gradient text-white pt-28 pb-14">
    <div class="container mx-auto px-6">
        <h1 class="text-3xl md:text-5xl font-extrabold tracking-tight">{{ $aboutSetting->title ?? 'About Wise Dynamic' }}</h1>
        <p class="mt-3 text-white/90 max-w-3xl">{{ $aboutSetting->subtitle ?? 'We craft high-performing digital products and growth campaigns that help businesses move faster, scale smarter, and convert better.' }}</p>
    </div>
</header>
    ...
    <div class="container mx-auto px-6 grid lg:grid-cols-2 gap-10 items-center">
        ...
        <div>
            <div class="rounded-2xl overflow-hidden shadow-xl">
                <img src="{{ $aboutSetting->who_we_are_image ?? 'https://images.unsplash.com/photo-1551836022-d5d88e9218df?q=80&w=1600&auto=format&fit=crop' }}" alt="Team at work" class="w-full h-80 object-cover">
            </div>
        </div>
    </div>
</section>
...
<section class="py-10 bg-gradient-to-br from-blue-50 to-purple-50">
    <div class="container mx-auto px-6 grid md:grid-cols-3 gap-6">
        @if(isset($aboutSetting->stats) && is_array($aboutSetting->stats))
            @foreach($aboutSetting->stats as $stat)
                <div class="bg-white rounded-2xl p-6 shadow text-center">
                    <div class="text-3xl font-extrabold gradient-text">{{ $stat['value'] ?? '' }}</div>
                    <div class="text-gray-700 mt-1">{{ $stat['label'] ?? '' }}</div>
                </div>
            @endforeach
        @else
            <div class="bg-white rounded-2xl p-6 shadow text-center">
    ...
    <div class="container mx-auto px-6 grid lg:grid-cols-2 gap-10 items-start">
        <div class="bg-white rounded-2xl shadow p-8">
            ...
            <ul class="mt-5 space-y-4">
                @if(isset($aboutSetting->values) && is_array($aboutSetting->values))
                    @foreach($aboutSetting->values as $value)
                        <li><span class="font-semibold text-gray-900">{{ $value['title'] ?? '' }}.</span> {{ $value['description'] ?? '' }}</li>
                    @endforeach
                @else
        ...
        <div class="bg-white rounded-2xl shadow p-8">
            ...
            <div class="mt-5 grid sm:grid-cols-2 gap-4">
                @if(isset($aboutSetting->services) && is_array($aboutSetting->services))
                    @foreach($aboutSetting->services as $service)
...
<section class="py-12 bg-gradient-to-r from-blue-600 to-purple-600">
    <div class="container mx-auto px-6 text-center text-white">
        <h3 class="text-2xl md:text-3xl font-extrabold">{{ $aboutSetting->cta_title ?? 'Ready to work with a results-driven team?' }}</h3>
        <p class="mt-2 text-white/90">{{ $aboutSetting->cta_subtitle ?? 'Get a free consultation and tailored plan for your business.' }}</p>
        <div class="mt-6">
            <a href="{{ route('contact') }}" class="inline-block bg-white text-gray-900 px-8 py-3 rounded-full font-semibold shadow hover:shadow-lg transform hover:-translate-y-0.5 transition">{{ $aboutSetting->cta_button_text ?? 'Contact Us' }}</a>
        </div>
    </div>
</section>
@endsection
