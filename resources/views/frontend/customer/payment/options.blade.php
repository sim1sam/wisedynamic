        ...
        <div class="p-6">
            <div class="grid md:grid-cols-3 gap-6">
                                    ...
                                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                        ...
                                        <div class="text-right">
                                            ...
                                            @if($payment->transaction_id)
                                                <p class="text-xs text-gray-500 mt-1">TXN: {{ $payment->transaction_id }}</p>
                                            @endif
                                        </div>
                                    </div>
                                @endforeach
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-green-600">৳{{ number_format($paymentStats['total_amount'], 2) }}</p>
                </div>
                @if($type === 'package')
                    <div>
                ...
                @if($paymentStats['is_partial_payment'])
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Paid Amount</label>
                        <p class="text-lg font-semibold text-blue-600">৳{{ number_format($paymentStats['paid_amount'], 2) }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Remaining</label>
                        <p class="text-lg font-bold text-orange-600">৳{{ number_format($paymentStats['remaining_amount'], 2) }}</p>
                    </div>
                @endif
            </div>
            ...
            @if($paymentStats['is_partial_payment'])
                ...
                <div class="mt-6 p-4 bg-blue-50 rounded-lg">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <span class="text-sm text-blue-700">{{ $paymentStats['payment_progress'] }}% Complete</span>
                    </div>
                    <div class="w-full bg-blue-200 rounded-full h-2">
                        <div class="bg-blue-600 h-2 rounded-full transition-all duration-300" style="width: {{ $paymentStats['payment_progress'] }}%"></div>
                    </div>
                    <p class="text-xs text-blue-700 mt-2">
                        You have paid ৳{{ number_format($paymentStats['paid_amount'], 2) }} out of ৳{{ number_format($paymentStats['total_amount'], 2) }}
                    </p>
                </div>
            @endif
        </div>
                ...
                <div class="border-2 border-blue-200 rounded-xl p-6 text-center hover:border-blue-400 hover:shadow-lg transition-all duration-300 bg-gradient-to-br from-blue-50 to-white">
                    ...
                    <form method="POST" action="{{ route('customer.payment.ssl', [$type, $order->id]) }}" id="ssl-payment-form">
                        ...
                        <button type="submit" class="w-full px-6 py-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:from-blue-700 hover:to-blue-800 transition-all duration-300 font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                            <i class="fas fa-credit-card mr-2"></i>Pay Now - BDT <span id="ssl-amount-display">{{ number_format($order->amount, 2) }}</span>
                        </button>
                    </form>
