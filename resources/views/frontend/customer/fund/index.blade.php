@php $title = 'Fund Management'; @endphp
    ...
    <div class="flex flex-col gap-2">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('customer.fund.create') }}" class="inline-flex items-center px-4 py-2 rounded-md bg-blue-600 text-white hover:bg-blue-700 shadow">
        ...
        <div class="p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-green-600">BDT {{ number_format(auth()->user()->balance, 2) }}</p>
                </div>
        ...
        <div class="p-6">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($fundRequests as $request)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">BDT {{ number_format($request->amount, 2) }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                            {{ $request->payment_method === 'ssl' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800' }}">
                                            {{ $request->payment_method === 'ssl' ? 'SSL Payment' : 'Manual Transfer' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500">{{ $request->created_at->format('M d, Y H:i') }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <a href="{{ route('customer.fund.show', $request) }}" class="text-indigo-600 hover:text-indigo-900">
                                            View Details
                                        </a>
                                    </td>
                                </tr>
            ...
            @else
                <div class="text-center py-8">
                    ...
                    <a href="{{ route('customer.fund.create') }}" class="inline-flex items-center px-4 py-2 rounded-md bg-blue-600 text-white hover:bg-blue-700">
