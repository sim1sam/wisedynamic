...
<section class="py-16 bg-gray-50">
    <div class="container mx-auto px-6">
        <div class="text-center mb-12">
            <h2 class="text-4xl font-bold mb-4 gradient-text">{{ $homeSetting->why_choose_title ?? 'Why Choose Wise Dynamic?' }}</h2>
            <div class="section-divider mb-6"></div>
            <p class="text-xl text-gray-700">{{ $homeSetting->why_choose_subtitle ?? 'We blend creativity, technology, and personalized support' }}</p>
        </div>
        
        <div class="grid lg:grid-cols-2 gap-12 items-center">
            <div class="space-y-6">
                @if(isset($homeSetting->why_choose_items) && is_array($homeSetting->why_choose_items))
                    @foreach($homeSetting->why_choose_items as $item)
                        <div class="flex items-start space-x-4">
                            <div class="w-12 h-12 bg-white rounded-full flex items-center justify-center flex-shrink-0">
                                <i class="{{ $item['icon'] ?? 'fas fa-check' }} text-blue-600 text-xl"></i>
                            </div>
                            <div>
                                <h3 class="text-xl font-bold text-gray-900 mb-2">{{ $item['title'] ?? 'Feature' }}</h3>
            ...
            <div class="text-center">
                <div class="bg-white rounded-lg p-8 shadow-2xl animate-pulse-custom">
                    <div class="text-6xl font-bold gradient-text mb-4">{{ $homeSetting->why_choose_clients_count ?? '100' }}+</div>
                    ...
                    <div class="mt-8 grid grid-cols-2 gap-4 text-center">
                        <div>
                            <div class="text-2xl font-bold gradient-text">{{ $homeSetting->why_choose_experience ?? '4+ Years' }}</div>
