...
@section('content')
<div class="card">
  <div class="card-header"><h3 class="card-title">New Slide</h3></div>
  <form action="{{ route('admin.slides.store') }}" method="post" enctype="multipart/form-data">
    ...
    <div class="card-body">
      ...
      <div class="form-group">
        ...
        <input type="text" name="price_text" class="form-control" value="{{ old('price_text') }}" placeholder="e.g. Starting from BDT 20,000/-">
      </div>
      <div class="form-group">
        <label>Link URL (optional)</label>
        <input type="url" name="link_url" class="form-control" value="{{ old('link_url') }}" placeholder="https://example.com/landing">
      </div>
      <div class="form-row">
        <div class="form-group col-md-4">
          <label>Image Source</label>
          <select name="image_source" id="image_source" class="form-control">
            <option value="url" {{ old('image_source','url')==='url'?'selected':'' }}>URL</option>
            <option value="upload" {{ old('image_source')==='upload'?'selected':'' }}>Upload</option>
          </select>
        </div>
      ...
      <div class="form-row">
        ...
        <div class="form-group col-md-3 d-flex align-items-center">
          <div class="custom-control custom-switch mt-4">
            <input type="checkbox" name="active" class="custom-control-input" id="activeSwitch" {{ old('active',true)?'checked':'' }}>
